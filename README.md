...
- 我在頁面加入了輕量動畫（元素淡入、按鈕彈跳、頭像脈動），以改善頁面動態感。
- 為了性能與可存取性，動畫在以下情況會暫時關閉：
	- 手機小螢幕（寬度 < 600px）會關閉大部分動畫；
	- 使用者系統偏好設定了「減少動態效果」(prefers-reduced-motion: reduce)。
- reveal-on-scroll 使用 IntersectionObserver 驅動，且在物件首次出現後即停止觀察以節省記憶體與效能。
